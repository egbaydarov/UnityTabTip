...
public class GvrLaserPointer : GvrBasePointer
{
    ...
    private void Awake()
    {
        ...
        {
            enabled = false;
            Debug.LogError("Behanvior pose not configured. Disabling...");
        }
    }
    ...
    public override bool TriggerDown => SteamVR_Input.GetBooleanAction("InteractUI").GetState(pose.inputSource);
    public override bool TriggerUp => !SteamVR_Input.GetBooleanAction("InteractUI").GetState(pose.inputSource);
    public override bool Triggering => SteamVR_Input.GetBooleanAction("InteractUI").GetState(pose.inputSource);
}
